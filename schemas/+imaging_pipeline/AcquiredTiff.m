...
classdef AcquiredTiff < dj.Imported
    ...
    methods(Access=protected)
        ...
        function insert_scan_info(self, key, recInfo, bucket_dir)
            ...
            originalkey                   = key;
            key_data                      = fetch(imaging_pipeline.ImagingPipelineSession & originalkey);
            key                           = key_data;
        ...
        function insert_fov_mesoscope(self, fl, key_data, skipParsing, imheader, recInfo, basename, cumulativeFrames, scan_dirs_db)
            ...
            for iROI = 1:nROI
                ...
                for iZ = 1:ndepths
                    ...
                    fov_key.tiff_split_directory = sprintf('%s/ROI%02d_z%d/',scan_dirs_db.recording_directory,iROI,iZ);
                    %[~,~,fov_key.relative_fov_directory] = lab.utils.get_path_from_official_dir(fov_key.fov_directory);
                    %fov_key.relative_fov_directory = fov_key.fov_directory;
                    ...
                    if isempty(fov_key.fov_center_xy) || isnan(fov_key.fov_center_xy)
                        fov_key.fov_center_xy  = -1;
                    end
                    
                    fov_key.fov_size_xy             = recInfo.ROI(iROI).sizeXY;
                    if isempty(fov_key.fov_size_xy) || isnan(fov_key.fov_size_xy)
                        fov_key.fov_size_xy = -1;
                    end
                    
                    fov_key.fov_rotation_degrees    = recInfo.ROI(iROI).rotationDegrees;
                    if isempty(fov_key.fov_rotation_degrees) || isnan(fov_key.fov_rotation_degrees) 
                        fov_key.fov_rotation_degrees = -1;
                    end
                    
                    fov_key.fov_pixel_resolution_xy = recInfo.ROI(iROI).pixelResolutionXY;
                    if isempty(fov_key.fov_pixel_resolution_xy) || isnan(fov_key.fov_pixel_resolution_xy)
                    ...
                    if isempty(fov_key.fov_discrete_plane_mode) || isnan(fov_key.fov_discrete_plane_mode)
                    ...
                    file_entries.tiff_split         = fov_key.tiff_split;
                    file_entries.file_number        = [];
                    file_entries.tiff_split_filename= '';
        ...
        function insert_fov_photonmicro(self, key, recInfo, scan_dirs_db)
            ...
            insert(imaging_pipeline.TiffSplit, fovkey)
            ...
            if(~isempty(fl))
                ...
                for iF = 1:numel(fl)
                    ...
                    if (length(acq_string) == 1 && length(number_string) == 1)
                        ...
                        filekeys(iF).tiff_split_filename = fl{iF};
                ...
                insert(imaging_pipeline.TiffSplitFile, filekeys)
            end
