...
sync_behav_iter_by_im_frame        :    longblob   # array with behavioral iteration or frame for each imaging frame, some extra zeros in file 1, marking that the behavior recording hasn't started yet.
...
classdef SyncImagingBehavior < dj.Computed
  ...
  methods(Access=protected)
    ...
    function makeTuples(self, key)
      ...
      %% behav
      % get behavior recording key
      
      
      session_key    = fetch(imaging_pipeline.ImagingPipelineSession * ...
      ...
      % path
      fov_bucket_directory          = fetch1(imaging_pipeline.TiffSplit & key,'tiff_split_directory');
      config                        = dj.config;
      imaging_root_dir              = config.custom.ImagingRootDataDir{1};
      fov_directory                 = fullfile(imaging_root_dir, fov_bucket_directory);

      %Check if directory exists in system
      lab.utils.assert_mounted_location(fov_directory)

      [order,movieFiles]            = fetchn(imaging_pipeline.TiffSplitFile & key, 'file_number', 'tiff_split_filename');
