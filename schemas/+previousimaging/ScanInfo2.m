...
classdef ScanInfo2 < dj.Imported
  ...
  methods(Access=protected)
    ...
    function makeTuples(self, key)
      ...
      generalTimer   = tic;
      curr_dir       = pwd; 
      scan_directory = formatFilePath(fetch1(previousimaging.Scan & key,'scan_directory'),true,true);
...
%       lastFrameInFile     = lastGoodFrame - cumulativeFrames(max([1 lastGoodFile-1]));
      ...
      %% write to this table
      originalkey                   = key;
      key_data                      = fetch(previousimaging.Scan & originalkey);
      key                           = key_data;
  ...
  %                 if iF == lastGoodFile && iZ > lastFrameInFile; continue; end
      ...
      for iROI = 1:nROI
        ...
        for iZ = 1:ndepths
          ...
          ct = ct+1;
          inserti(previousimaging.FieldOfView,fov_key)
          ...
          insert(previousimaging.FieldOfViewFile, file_entries)
        end
      end
