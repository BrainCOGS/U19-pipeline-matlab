    ...
    methods
        ...
        function insertNewSessionStarted(self, key, task, localPath, trainingRig)
            ...
            if status
                ...
                %If stored in local machine in C:\DATA
                if contains(localPath, self.LOCALPATHTOKEN)
                    remote_location = lab.utils.get_corresponding_remote_location(...
                        self.LOCALPATHTOKEN, localPath, default_rig_path_location);
                %If not, presume is in network 
                else
                    [~,network_path_token] = lab.utils.get_path_from_official_dir(location_info.bucket_default_path);
                    if contains(localPath, network_path_token)
                        remote_location = lab.utils.get_corresponding_remote_location(...
                            network_path_token, localPath, default_rig_path_location);
                    end
                end
