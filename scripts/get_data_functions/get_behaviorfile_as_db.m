function data_struct = get_behaviorfile_as_db(key, selected_columns)
%get_behaviorfile_as_db rearrange behavior file to output as Behavior tables
% Inputs
% key = = Session keys to fetch data from
% selected_columns = If only selected columns are needed from beh files
% Outputs
        ...
        for iBlock = 1:length(log.block)
            ...
            if nTrials > 0
                ...
                if ~isempty(selected_columns)
                    session_data = session_data(:,selected_columns);
                end
                
                %Fix trialProb variable (if multiple sessions in a single file
                if ismember('trialProb', session_data.Properties.VariableNames);
