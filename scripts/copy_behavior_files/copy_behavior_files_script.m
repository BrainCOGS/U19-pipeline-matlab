...
try
    ...
    log_scheduled_task_event(task_name, 'INFO', ...
        sprintf('rig=%s: starting behavior backup copy', rig));

    [final_status, msg] = copy_remote_behavior_files(rig);
    if ~final_status
        successful_task = 0;
        notify_scheduled_task_failure(task_name, rig, ...
            sprintf('copy_remote_behavior_files failed: %s', msg));
    else
        log_scheduled_task_event(task_name, 'INFO', ...
            sprintf('rig=%s: copy_remote_behavior_files completed successfully', rig));
    end
catch err
    successful_task = 0;
    try
...
if successful_task
    log_scheduled_task_event(task_name, 'INFO', ...
        sprintf('rig=%s: behavior backup copy task completed successfully', rig));
end
